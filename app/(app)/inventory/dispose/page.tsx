...
export default function DisposePage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!lookup) {
      setErrorMessage("Load on-hand inventory before saving disposal.");
      return;
    }

    if (!Number.isFinite(numericQuantity) || numericQuantity <= 0) {
      setErrorMessage("Enter a valid disposal quantity.");
      return;
    }

    if (numericQuantity > Number(lookup.onHand || 0)) {
      setErrorMessage("No on-hand inventory available for this disposal amount.");
      return;
    }

    if (!normalizeText(reasonCode)) {
      setErrorMessage("Select a disposal reason.");
      return;
    }

    if (!witnessSatisfied) {
      setErrorMessage("Witness is required for DEA scheduled medication disposal.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        setErrorMessage(data?.error || "Failed to dispose medication");
        return;
      }

      setSuccessMessage("Disposal recorded successfully.");
      setQuantity("");
      setNote("");
      setWitnessId("");
      setWitnessName("");
      ...
      console.error("Failed to dispose medication", error);
      setErrorMessage("Failed to dispose medication");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-semibold text-slate-900">Dispose Medication</h1>

...
export async function POST(req: Request) {
  try {
    ...
    const body = (await req.json()) as WasteRequestBody;
    ...
    if (!Number.isFinite(numericQuantity) || numericQuantity <= 0) {
      return NextResponse.json(
        { error: "Valid waste quantity is required" },
        { status: 400 }
      );
    }

    if (!reasonCode) {
      return NextResponse.json({ error: "Waste reason is required" }, { status: 400 });
    }
    ...
    if (controlled) {
      if (!witnessUserId && !witnessName) {
        return NextResponse.json(
          { error: "Witness is required for DEA scheduled medication waste." },
          { status: 400 }
        );
      }
      ...
      if (witnessUserId) {
        ...
        if (witnessUser.id === actor.id) {
          return NextResponse.json(
            { error: "Witness cannot be the same user performing the waste" },
            { status: 400 }
          );
        }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      let remainingToWaste = numericQuantity;
      ...
      for (const item of inventoryItems) {
        ...
        const deduction = Math.min(itemQty, remainingToWaste);
    ...
    return NextResponse.json({
      ...
      quantityWasted: numericQuantity,
    ...
    return NextResponse.json(
      { error: "Failed to waste medication" },
      { status: 500 }
    );
  }
}
